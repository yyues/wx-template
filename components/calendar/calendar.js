...
Component({
  ...
  methods: {
    ...
    selectChange(e) {
      ...
      console.log(this.currentWeek({ year, month, day }));
      this.setData({
        currentWeek: this.currentWeek({ year, month, day })
      })
    },
     ...
     */
    isNowDate(date) {
      const { year, month, day } = this.data.nowDay
      return !!(date.year === year && date.month === month && date.day === day)
    },
    // 计算今天周几并返回
    currentWeek(qqq) {
      const { year, month, day } = qqq
      const date = `${year}-${month}-${day}`
      const str = this.data.weekDay[new Date(date).getDay()]
      return str
    },
  },
